...
class CanvasDemo:
    def __init__(self,w,h):
        ...
        self.source_opt = srcoptions = {}  
        srcoptions['initialdir'] = 'C:\\'  
        srcoptions['mustexist'] = True  
        srcoptions['parent'] = self.window  
        srcoptions['title'] = 'Choose source folder'

        self.target_opt = tgtoptions = {}  
        tgtoptions['initialdir'] = 'C:\\'  
        tgtoptions['mustexist'] = True  
        tgtoptions['parent'] = self.window  
        tgtoptions['title'] = 'Choose target folder'

        bm = PhotoImage(file='BG.gif')
        self.frame = PhotoImage(file='frame.gif')
        
        #bg img and frame img
        ...
        #bind callback to the mode and start button
        ...
        #bind callback to the mode button
        ...
        #label to show the directory choose by user
        self.pathlabel = self.canvas.create_text(w*0.196,h*0.48,text="Source File Path",font=("SF New Republic",20,""),anchor = NW,justify = CENTER)
        self.pathlabe2 = self.canvas.create_text(w*0.196,h*0.58,text="Target  File Path",font=("SF New Republic",20,""),anchor = NW,justify = CENTER)

        #
        self.buttomtext = self.canvas.create_text(w*0.189,h*3/4,text="The process may take a few minutes, please wait.",font=("SF New Republic",20,""),fill= "White",anchor = NW,justify = CENTER)
    ...
    def sourcedirectory(self):  
        ...
        self.target_opt['initialdir']=self.sourcepath
    ...
    def run(self):
        ...
        if (not os.path.isdir(self.sourcepath)):
            self.canvas.itemconfig(self.buttomtext,text = "Please choose the source directory")
            return
        print "test"
        self.canvas.itemconfig(self.buttomtext,text = "Loading...")
        ...
        self.canvas.itemconfig(self.buttomtext,text = "Management Completed")
